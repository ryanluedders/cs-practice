...
public class HolidaysTest {
    ...
    public void testBuildTree_can_fly_anywhere() {
        ...
        HolidayTree holidays = Holidays.buildTree(FLIGHT_TIMES, HOLIDAYS_PER_WEEK, maxFlightTime);
        
        System.out.println(holidays.toString());
        
        assertNotNull(holidays);
        assertEquals(364, holidays.size()); // 3^5 + 3^4 + 3^3 + 3^2 + 3^1 + 3^0
        assertEquals(7, Holidays.findMaximumPath(holidays));
    }
    ...
    public void testBuildTree_can_fly_within_7_hours() {
        ...
        HolidayTree holidays = Holidays.buildTree(FLIGHT_TIMES, holidaysPerWeek, maxFlightTime);
        
        System.out.println(holidays.toString());
        
        assertNotNull(holidays);
        assertEquals(34, holidays.size()); // 1 + 3 + 9 + 21 = 34
        assertEquals(2, Holidays.findMaximumPath(holidays));
    }
